...
% Define the System Matrices
 ...
 Cm1=[kc 0 0 0 ;0 kc 0 0]; %Change according to the controller (To control)
 Co1=[0 0 kc 0 ;0 0 0 kc]; %Change according to the observer (To measure)
 Dm1=[0 0;0 0];
...
%For Kalman Filter Implementation
 ...
 %trposteriori(1,k) = trace(P_post(:,:,k));
  ...
  Xf=[xm-xm_old;y_mpc]; %full state feedback in delta
    ...
    k=0:Ts:(nlen-1)*Ts;
    figure;                    %Plots all the tank measurements with legend
    for i = 1:4
  ...
  figure;
    subplot(311)                          %Plots the controlled heights only.
        plot(transpose(y1(1,:)),'displayname',strcat('h1'));
        hold on;
        plot(transpose(y1(2,:)),'displayname',strcat('h2'));
    xlabel('Time')
