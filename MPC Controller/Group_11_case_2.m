...
% Define the System Matrices
 ...
 Cm1=[0 0 kc 0 ;0 0 0 kc]; %Change according to the controller (To control)
 Co1=[kc 0 0 0 ;0 kc 0 0]; %Change according to the observer (To measure)
 Dm1=[0 0;0 0];
 ...
 %trposteriori(1,k) = trace(P_post(:,:,k));
  ...
  figure;
    subplot(311)                          %Plots the controlled heights only.
        plot(transpose(y1(1,:)),'displayname',strcat('h3'));
        hold on;
        plot(transpose(y1(2,:)),'displayname',strcat('h4'));
    xlabel('Time')
